...
class FakeRequest:
    def __init__(
        ...
        *,
        ...
        path: str = "/pipeline/audio-generate-language",
    ) -> None:
        self.method = method
...
def test_audio_generate_language_uses_deterministic_stub(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def _raise_executor() -> None:
        raise AssertionError("Executor must not be called when PIPELINE_AUDIO_E2E_STUB=true")

    monkeypatch.setattr(functions_main, "get_executor", _raise_executor)

    req = FakeRequest(body={
        "sessionId": "audio-stub-1",
        "voiceId": "Aoede",
        "language": "ja",
        "scripts": [
            {"spotId": "spot_001", "spotNumber": 1, "title": "Spot 1", "scriptText": "Hello world"},
            {"spotId": "spot_002", "spotNumber": 2, "title": "Spot 2", "scriptText": "Second paragraph"},
        ],
        "translations": [
            {"spotId": "spot_001", "translatedText": "こんにちは世界"},
            {"spotId": "spot_002", "translatedText": "二つ目の段落"},
        ],
    })
...
def test_audio_generate_language_stub_still_enforces_session_tenant_scope(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        functions_main.session_service,
        "get_session",
        lambda _sid: {"context": {"tenantId": "tenant-b"}},
    )

    req = FakeRequest(body={
        "sessionId": "audio-stub-tenant-check",
        "voiceId": "Aoede",
        "language": "en",
        "scripts": [
            {"spotId": "spot_001", "spotNumber": 1, "title": "Spot 1", "scriptText": "Hello world"},
    ...
    payload = _extract_json(response)
    assert payload["error"]["code"] == "FORBIDDEN_SESSION_TENANT_MISMATCH"

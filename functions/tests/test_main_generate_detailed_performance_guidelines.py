...
class FakeRequest:
    def __init__(
        ...
        *,
        ...
        path: str = "/pipeline/generate-detailed-performance-guidelines",
    ) -> None:
        self.method = method
...
def test_generate_detailed_performance_guidelines_calls_executor(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
    ...
    class _Executor:
        async def generate_detailed_performance_guidelines(
            ...
            *,
            ...
            assert where == "A quiet shrine at dusk."
            assert who == "First-time visitors."
            assert what == "Create a feeling of reverence."
            assert how == "Softly, with respectful pauses."
            ...
            return {
                ...
                "detailedPerformanceGuidelines": "\n".join([
                    'Style: The "Reverent Guide". The tone carries contained awe and a composed facial softness.',
                    'Pace: Speaks with a calm, unhurried cadence and deliberate 1-second pauses after key facts.',
                    'Accent: Use a refined neutral English accent with precise museum-guide diction.',
                ]),
            }
    ...
    response = functions_main.generate_detailed_performance_guidelines(FakeRequest(body={
    ...
    assert payload["detailedPerformanceGuidelines"].startswith('Style: The "Reverent Guide".')


def test_generate_detailed_performance_guidelines_rejects_invalid_body(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
        "_authorise_admin_request",
        lambda req, require_tenant_scope=False: ({

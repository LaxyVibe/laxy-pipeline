...
class FakeRequest:
    def __init__(
        ...
        *,
        ...
        path: str = "/pipeline/generate-character",
    ) -> None:
        self.method = method
...
def test_generate_character_calls_executor_with_structured_fields(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
    ...
    class _Executor:
        async def generate_character(self, *, name: str, gender: str, role: str, context: str) -> dict[str, Any]:
            assert name == "John"
            assert gender == "Male"
            assert role == "Museum Manager"
            assert context == "A knowledgeable person who has a formal and confident tone."
            return {
                "success": True,
                "character": {
                    "name": name,
    ...
    response = functions_main.generate_character(FakeRequest(body={
        "name": "John",
        "gender": "Male",
        "role": "Museum Manager",
    ...
    assert payload["character"]["audioProfileMarkdown"].startswith("# AUDIO PROFILE: John")


def test_generate_character_rejects_invalid_body(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
        "_authorise_admin_request",
        lambda req, require_tenant_scope=False: ({

...
class FakeRequest:
    def __init__(
        ...
        *,
        ...
        path: str = "/pipeline/generate-japanese-hiragana",
    ) -> None:
        self.method = method
...
def test_generate_japanese_hiragana_calls_executor(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
    ...
    class _Executor:
        async def generate_japanese_hiragana(self, *, script_content: str) -> dict[str, Any]:
            assert script_content == "明日、軽やかに風景を描く。"
            return {"success": True, "hiraganaText": "あす、かろやかにふうけいをえがく。"}
    ...
    response = functions_main.generate_japanese_hiragana(req)

    assert _extract_status(response) == 200
    payload = _extract_json(response)
    assert payload["success"] is True
    assert payload["hiraganaText"] == "あす、かろやかにふうけいをえがく。"


def test_generate_japanese_hiragana_rejects_invalid_body(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        functions_main,
        "_authorise_admin_request",
        lambda req, require_tenant_scope=False: ({
    ...
    response = functions_main.generate_japanese_hiragana(FakeRequest(body={}))
